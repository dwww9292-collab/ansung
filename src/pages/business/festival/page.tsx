...
export default function FestivalPage() {
  ...
  const { items, loading } = usePublicEvents("festival");
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-center text-gray-900 mb-10">축제</h2>
      ...
      <EventHero
        ...
        eyebrow="2026 안성 축제"
        fallbackTitle="축제"
        fallbackDesc="안성 대표 공연예술축제로, 안성 음악축제·안성 시민공원 불꽃축제 등을 통해 보편적 문화 복지를 실현하고 다양한 공연 콘텐츠로 시민들과 함께 하겠습니다."
      />
      ...
      <div className="px-4 md:px-8 pt-8">
        ...
        {!loading && filtered.length === 0 && (
          <div className="py-20 text-center text-gray-400 text-sm">등록된 축제가 없습니다.</div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {filtered.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(`/business/festival/${item.id}`)}
            ...
            >
              <div className="relative w-full h-[200px]">
                <div className="absolute top-2 left-2 z-10">
                  <span className={`px-2 py-0.5 text-xs font-bold rounded ${tagColor[item.status] || "bg-[#999]"} text-white`}>
                    {item.status}
                  </span>
                </div>
                <img
