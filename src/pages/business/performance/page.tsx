...
export default function PerformancePage() {
  ...
  const { items, loading } = usePublicEvents("performance");
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-center text-gray-900 mb-10">공연</h2>

      <EventHero
        section="performance"
        activeHref="/business/performance"
        fallbackTitle="공연"
        fallbackDesc="다채롭고 수준높은 공연 제공을 통해 지역의 문화 향유권을 확대하고, 시민과 동행하는 문화도시 안성로 나아가고자 합니다."
      />
      ...
      <div className="px-4 md:px-8 pt-8">
        ...
        {!loading && filtered.length === 0 && (
          <div className="py-20 text-center text-gray-400 text-sm">등록된 공연이 없습니다.</div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {filtered.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(`/business/performance/${item.id}`)}
            ...
            >
              <div className="relative w-full h-[200px]">
                <div className="absolute top-2 left-2 z-10">
                  <span className={`px-2 py-0.5 text-xs font-bold rounded ${tagColor[item.tag] || "bg-gray-500"} text-white`}>
                    {item.tag}
                  </span>
                </div>
                <img
